...
define :makeNoise do |i|
  ...
  depth[i] += ((rrand 2.0, 0.5) <=> depth[i]) * 0.1
  divisor[i] += ((rrand 1.5, 0.5) <=> divisor[i]) * 0.05
  note[i] += ((rrand 30, 40) <=> note[i]) * 0.2
  pan[i] += ((rrand -0.5, 0.5) <=> pan[i]) * 0.1
  amp[i] += ((rrand 0.5, 1.2) <=> amp[i]) * 0.1
